...
func fieldsToWire(set field.FieldSet) []wire.Field {
	var r []wire.Field
...
func labelsToWire(set label.LabelSet) []string {
	var r []string
	l := set.Labels()
	for i := range l {
		r = append(r, l[i].Key, l[i].Value)
	}
	return r
}
...
func wireToFields(wfs []wire.Field) field.FieldSet {
	var fs []field.Field
	for _, wf := range wfs {
		switch wf.FieldType {
		case wire.FieldTypeString:
			fs = append(fs, field.String(wf.Key, wf.GetStringVal()))
		...
		case wire.FieldTypeBool:
			...
			fs = append(fs, field.Bool(wf.Key, boolVal))

		case wire.FieldTypeInt64:
			fs = append(fs, field.Int64(wf.Key, wf.GetNumericVal()))

		case wire.FieldTypeUint64:
			fs = append(fs, field.Uint64(wf.Key, uint64(wf.GetNumericVal())))

		case wire.FieldTypeDuration:
			fs = append(fs, field.Duration(wf.Key, time.Duration(wf.GetNumericVal())))

		case wire.FieldTypeFloat64:
			fs = append(fs, field.Float64(wf.Key, math.Float64frombits(uint64(wf.GetNumericVal()))))
		}
	}

	return field.Fields(fs...)
}
